...
<head>
                ...
                function callback(data) {
                    ...
                    if (data == null) {
                            ...
                            for (var b = 0; b < data[k].length; b++) {
                                if (b == 0) {
                                    ...
                                    st += "<td>" +
                                        ...
                                        "<td align='center'>" +
                                        "<div class='c_num'>" +
                                        "<input type='button' value='' onclick='jianProduct(jq(this));' class='car_btn_1'/>" +
                                        "<input type='text' id='" + data[k][b].id + "'  value='" + data[k].length + "'  name='' class='car_ipt'/>" +
                                        "<input type='button' value='' onclick='jiaProduct(jq(this)," + data[k][b].stock + ");' class='car_btn_2'/>" +
                                        "</div>" +
                                        "</td>" +
                                        "<td align='center' style=color:#ff4e00;>￥" + (data[k][b].price * data[k].length) + "</td>" +
                                        "<td align='center'>26R</td>" +
                                        "<td align='center'><a id='" + data[k][b].id + "'  onclick='shanProduct(jq(this))'>删除</a>&nbsp; &nbsp;<a href=#>加入收藏</a>" +
                                        "</td>" +
                                        "</tr>";
                                }
                            }
                        ...
                        st += "<tr height='70'>" +
                            ...
                            "                    <label class='r_rad'><input type='button' name='clear' class='r_txt' value='清空购物车' onclick='deleteAll()'/></label>" +
                            "                    <span class='fr' id='pri'>商品总价：<b style='font-size:22px; color:#ff4e00;'>" + sum + "</b></span>\n" +
                            "                </td>" +
                            "            </tr>" +
                            "            <tr valign='top' height='150'>\n" +
                            "                <td colspan='6' align='right'>\n" +
                            "                    <a href='index.jsp'><img src='images/buy1.gif'/></a>&nbsp; &nbsp; <a href='/UserServlet?opr=selectByUserId&userId=" + userId + "'><img\n" +
                    ...
                    for (var b = 0; b < data[k].length; b++) {
                        if (b == 0) {
                            ...
                            st += "<td>" +
                                ...
                                "<td align='center' style=color:#ff4e00;>￥" + (data[k][b].price * data[k].length) + "</td>" +
                                "<td align='center'>26R</td>" +
                                "<td align='center'><a id='" + data[k][b].id + "'  onclick='shanProduct(jq(this))'>删除</a>&nbsp; &nbsp;" +
                                "<a href='/ProductCategoryServlet?opr=doCollect&userId=" + userId + "&productId=" + data[k][b].id + "'>加入收藏</a>" +
                                "</td>" +
                                "</tr>";
                        }
                    }
                    c++;
                    sum += (data[k][0].price * data[k].length);
                }
    ...
    <script>
        //减少商品或删除商品
        function jianProduct(jian) {
            (function ($) {
                var num = parseInt(jian.next().val());
                var id = jian.next().attr("id");
                var loginName = "${sessionScope.eu.loginName}";
                if (num > 1) {
                    num = num - 1;
                    $.post("/ProductCategoryServlet?opr=reduce", "loginName=" + loginName + "&id=" + id, callback, "json");
                    ...
                    function callback(data) {
                        ...
                        $("#pri").children("b").html("￥" + sum);
                        jian.next().val(num);
                        jian.parent().parent().next().html("￥" + (data[id][0].price) * num);
                    }
                } else if (num == 1) {
                    if (window.confirm("这是最后一件商品了，确定要删除该商品吗？")) {
                        $.post("/ProductCategoryServlet?opr=delete", "loginName=" + loginName + "&id=" + id, callback, "json");
    ...
    <script>
        //添加商品
        function jiaProduct(jia, stock) {
            (function ($) {
                if (${sessionScope.eu == null}) {
                    alert("请先登录");
                } else {
                    if (stock > 0) {
                        var id = jia.prev().attr("id");
                        var loginName = "${sessionScope.eu.loginName}";
                        ...
                        function callback(data) {
                            ...
                            for (var key in data) {
                                ...
                                for (var j = 0; j < list.length; j++) {
                                    if (j == 0) {
                                        str += "<li>\n" +
                                            "                        <div class='img'><a href='#'><img src='files/" + list[j].fileName + "' width='58' height='58' /></a></div>\n" +
                                            "                        <div class='name'><a href='#'>" + list[j].name + "</a></div>\n" +
                                            "                        <div class='price'><font color='#ff4e00'>￥" + list[j].price + "</font> X" + list.length + "</div>\n" +
                                            "                    </li>";
                                    }
                                }
                                prices = list[0].price * list.length;
                                sum += prices;
                            }
                            str += "</ul>\n" +
                                "                <div class='price_sum'>共计&nbsp; <font color='#ff4e00'>￥</font><span>" + sum + "</span></div>\n" +
                            ...
                            $car.html(str);
                            jia.prev().val(num);
                            jia.parent().parent().next().html("￥" + (data[id][0].price) * num);
                            $("#pri").children("b").html("￥" + sum);
                        }
                    } else {
                        alert("该商品库存不足，请选择其他商品");
                    }
                }
            })(jQuery)
        }
                    ...
                    function callback(data) {
                        ...
                        if (data == null) {
                            str += "<div class='car_t'>购物车 [ <span>0</span> ]</div>" +
                                ...
                                "      <ul class='cars'></ul>\n" +
                                "                <div class='price_sum'>共计&nbsp; <font color='#ff4e00'>￥</font><span>0</span></div>\n" +
                                "                <div class='price_a'><a href='/ProductCategoryServlet?opr=getProduct&num=0&userName=" + userName + "'>去购物车结算</a></div>\n" +
                                "                </div>";
                        } else {
                            var length = 0;
                            for (var key1 in data) {
            ...
            (function ($) {
                ...
                function callback(data) {
                    if (data == "success") {
                        ...
                        var $cat_tab = $("#car_tab");
                        $cat_tab.remove();
                        if (window.confirm("购物车中商品已清空，请先前往主页面购买商品")) {
...
<div class="i_bg">
    ...
    <div class="content mar_20">
        <table border="0" id="car_tab" class="car_tab" style="width:1200px; margin-bottom:50px;" cellspacing="0"
               cellpadding="0">
            <tr>
                <td class="car_th" width="490">商品名称</td>
                <td class="car_th" width="140">属性</td>
            ...
            <c:forEach items="${requestScope.map}" var="map" varStatus="i">
                <c:forEach items="${map.value}" var="item" begin="0" end="0">
                    <tr <c:if test="${i.count%2==0}">class="car_tr"</c:if>>
                        <td>
                            <div class="c_s_img"><img src="files/${item.fileName}" width="73" height="73"/></div>
                                ${item.name}
                        </td>
                        <td align="center">颜色：灰色</td>
                        <td align="center">
                            <div class="c_num">
                                <input type="button" value="" onclick="jianProduct(jq(this));" class="car_btn_1"/>
                                <input type="text" id="${item.id}" value="${fn:length(map.value)}" name=""
                                       class="car_ipt"/>
                                <input type="button" value="" onclick="jiaProduct(jq(this),${item.stock});"
                                       class="car_btn_2"/>
                            </div>
                        </td>
                        <td align="center" style="color:#ff4e00;">￥${(fn:length(map.value)) * item.price}</td>
                        <td align="center">26R</td>
                        <td align="center"><a id="${item.id}" onclick="shanProduct(jq(this))">删除</a>&nbsp; &nbsp;<a
                                href="/ProductCategoryServlet?opr=doCollect&userId=${sessionScope.eu.id}&productId=${item.id}">加入收藏</a>
                        </td>
                    </tr>
                </c:forEach>
            </c:forEach>
            ...
            <tr height="70">
                <td colspan="6" style="font-family:'Microsoft YaHei'; border-bottom:0;">
                    <label class="r_rad"><input type="button" name="clear" class="r_txt" value="清空购物车" onclick="deleteAll()"/></label>
                    <span class="fr" id="pri">商品总价：<b style="font-size:22px; color:#ff4e00;"></b></span>
                </td>
            </tr>
            <tr valign="top" height="150">
                <td colspan="6" align="right">
                    <a href="index.jsp"><img src="images/buy1.gif"/></a>&nbsp; &nbsp; <a
                        href="/UserServlet?opr=selectByUserId&userId=${sessionScope.eu.id}"><img
                        src="images/buy2.gif"/></a>
                </td>
            </tr>
        </table>
