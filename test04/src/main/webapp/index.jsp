...
<head>
    ...
    <script>
        //页面加载就执行
        $(function () {
            /*$.post("/ProductCategoryServlet?opr=pclist");*/
...
<div class="top">
    <div class="logo"><a href="/ProductCategoryServlet?opr=pclist"><img src="images/logo.png"/></a></div>
    <div class="search">
        <form>
...
<div class="i_bg bg_color">
                    ...
                    $(function () {
                        ...
                        function callback(data) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                $news.append("<li><span>" + str + "</span><a href='#'>" + data[i].title + "</a></li>");
                            }
    ...
    <c:forEach items="${sessionScope.list}" var="temp1" varStatus="i">
        <div class="i_t mar_10">
            <span class="floor_num">${i.count}F</span>

            <span class="fl">
                <c:set var="str" value="${temp1.name}"/>
                <c:set var="index" value="${fn:indexOf(temp1.name,',')}"/>
                <c:set var="length" value="${fn:length(temp1.name)}"/>
                <c:choose>
                    <c:when test="${fn:contains(str,',')}">
                        ${fn:substring(temp1.name,0 ,index )}<b>·</b>${fn:substring(temp1.name,index+1 ,length)}
                    </c:when>
                    <c:otherwise>
                        ${temp1.name}
                    </c:otherwise>
            ...
            <span class="i_mores fr">
                <c:forEach items="${temp1.list}" var="temp2" varStatus="j">
                    <a href="javascript:;" id="${temp2.id}" onclick="go(${temp2.id},2,1)">${temp2.name}</a>
                    <c:set var="cou" value="${fn:length(temp1.list)}"/>
                    <c:if test="${j.count < cou}">
                        &nbsp; &nbsp; &nbsp;
                    </c:if>
                </c:forEach>
            </span>
        ...
        <div class="content">
            <div class="fresh_left">
                ...
                <div class="fresh_txt">
                    <div class="fresh_txt_c">
                        <c:forEach items="${temp1.list}" var="temp2">
                            <c:forEach items="${temp2.list}" var="temp3">
                                <a href="javascript:;" id="${temp3.id}" onclick="go(${temp3.id},3,1)">${temp3.name}</a>
                            </c:forEach>
                        </c:forEach>
            ...
            <div class="fresh_mid">
                <ul>
                    <c:forEach items="${temp1.list}" var="temp2">
                        <c:forEach items="${temp2.list}" var="temp3">
                            <c:forEach items="${temp3.plist}" var="temp4">
                                <li>
                                    <div class="name"><a
                                            href="/ProductCategoryServlet?opr=product&id=${temp4.id}">${temp4.name}</a>
                                    </div>
                                    <div class="price">
                                        <font>￥<span>${temp4.price}</span></font> &nbsp; 26R
                                    </div>
                                    <div class="img"><a href="/ProductCategoryServlet?opr=product&id=${temp4.id}"><img
                                            src="files/${temp4.fileName}" width="185" height="155"/></a></div>
                                </li>
                            </c:forEach>
                        </c:forEach>
                    </c:forEach>
